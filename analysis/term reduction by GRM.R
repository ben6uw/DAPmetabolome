...
#################################################################################################
load('data/metabolome/ProcessedData/P1.metabolome.withBreedData') # see 'signalment.R
...
table(d$dog_id %in% rownames(grm))
d <- d[d$dog_id %in% rownames(grm), ] # g is now composed of variables that have been scaled
grm <- grm[d$dog_id, d$dog_id] # subset and order the GRM by the dogs in the mz data

###################################

Zmat <- diag(nrow(d)) # an empty design matrix for the random effects
...
X <- model.matrix( ~ sqrtAge + sqrtWT + lifestage_at_DOC + sex + sterilization_status + hours_fasting + commonPurebred + krt_cbc_total_wbcs + krt_cbc_abs_bands + krt_cbc_abs_neutrophils + krt_cbc_abs_lymphocytes + krt_cbc_abs_monocytes + krt_cbc_abs_eosinophils + krt_cbc_abs_basophils + krt_cbc_rbc + krt_cbc_hgb + krt_cbc_hct + krt_cbc_mcv + krt_cbc_mch + krt_cbc_mchc + krt_cbc_rdw + krt_cbc_mpv + krt_cbc_pct + krt_cbc_retic_abs, data=d) 
...
save(mList, nullmList, d, pcVars, X, sigEigs, file='dog metabolome paper/effect_of_GRM_results')
...
variances <- data.frame(PC=pcVars, 'PCvariance'= apply(d[ ,pcVars], 2, var), 'grm'=grmV, 'naive'=naiveV)
head(variances)
...
ggplot(l, aes(y=variance/PCvariance, x=PC, fill=model))+
  ...
  ylab('PC variance explained by 12 common breeds')

# this plot is misleading in the sense that the variance explaiend by the GRM is huge in comparison to the amount of variation by breed that it accounts for

# incorporating the GRM reduced the variance explaned by the 12 common breeds among some, but not all PCs.  
# so, it could be that non-genetic aspects of the 12 common breeds are affecting the plasma metabolome.
...
VarList <- list("sqrtAge", "sqrtWT", "sexmale", "sterilization_statussterilie", "hours_fasting", lifeStageVars, breedVars, cbcVars)
     
m <- mList[[1]] # pull a model as an example

variance <- matrix(nr=16, nc=length(pcVars))
tmp <-matrix(nr=nrow(d), ncol=8)
...
l$PC <- factor(l$PC, levels=pcVars)
pcVariance <- apply(d[ ,pcVars], 2, var)
...
load('dog metabolome paper/removeGRMplot')

rm(list=ls())
################################################################################################
### term reduction by the inclusion of breed?    This seems a way to see what covars associate with the breed effect
#################################################################################################
load('data/metabolome/ProcessedData/P1.metabolome.withBreedData') # see 'signalment.R
...
Xnobreed <- model.matrix( ~ sqrtAge + sqrtWT + lifestage_at_DOC + sex + sterilization_status + hours_fasting + krt_cbc_total_wbcs + krt_cbc_abs_bands + krt_cbc_abs_neutrophils + krt_cbc_abs_lymphocytes + krt_cbc_abs_monocytes + krt_cbc_abs_eosinophils + krt_cbc_abs_basophils + krt_cbc_rbc + krt_cbc_hgb + krt_cbc_hct + krt_cbc_mcv + krt_cbc_mch + krt_cbc_mchc + krt_cbc_rdw + krt_cbc_mpv + krt_cbc_pct + krt_cbc_retic_abs, data=d) 
...
wbreed <- emmreml(y=pcDat[ ,1], X=X, Z=Zmat, K=grm, varbetahat = T, varuhat=T, PEVuhat=T, test=T) 
nobreed <- emmreml(y=pcDat[ ,1], X=Xnobreed, Z=Zmat, K=grm, varbetahat = T, varuhat=T, PEVuhat=T, test=T) 

clus <- makeCluster(n.cores) 
registerDoParallel(cores=n.cores)  
clusterExport(clus, varlist=c("pcDat", 'pcVars', 'X', 'Xnobreed', 'Zmat', 'grm'), envir = environment()) 

wbreedList <- list()
nobreedList <- list()

for(i in 1:length(pcVars)) {
  clusterExport(clus, varlist=c("pcDat", 'pcVars', 'X', 'Zmat', 'grm', 'Xnobreed'), envir = environment()) 
  Y <- pcDat[ ,i]
  wbreedList[[i]] <- emmreml(y=Y, X=X, Z=Zmat, K=grm, varbetahat = T, varuhat=T, PEVuhat=T, test=T) 
  nobreedList[[i]] <- emmreml(y=Y, X=Xnobreed, Z=Zmat, K=grm, varbetahat = T, varuhat=T, PEVuhat=T, test=T) }

save(nobreedList, wbreedList, d, pcVars, X, Xnobreed, file='dog metabolome paper/breedRemoval_results')



rm(list=ls())
##########################################
load('dog metabolome paper/breedRemoval_results')
##########################################

m <- wbreedList[[1]] # pull a model as an example
...
VarList <- list("sqrtAge", "sqrtWT", "sexmale", "sterilization_statussterilie", "hours_fasting", lifeStageVars, cbcVars)

variance <- matrix(nr=14, nc=length(pcVars))
tmp <-matrix(nr=nrow(d), ncol=7)
...
variance$term  <- factor(rep(c("age", "weight", "sex", "sterilization", "fasting", 'lifestage', 'cbc'), 2), levels=c("age", "weight", "sex", "sterilization", "fasting", 'lifestage', 'cbc'))

variance$model <- factor(c(rep('breed', 7), rep('without breed', 7)), levels=c('without breed', 'breed'))
head(variance) 
...
# look at the % change in variance explained +/- breed:

head(l)
w <- pivot_wider(l[ ,c(1:3, 6)], names_from=model, values_from=propVar)
w$pct.change <- ((w$breed-w$`without breed`)/w$`without breed`)*100
w$difference<- w$breed-w$`without breed`


ggplot(w, aes(y=pct.change, x=PC, fill=term))+
  geom_bar(stat = 'identity')+
  theme_classic(base_size = 14)+
  ylab('% chance in var. exp (breed - without / without)')+
  xlab('')+
  coord_flip()+
...
aggregate(pct.change ~ term, w, mean)

meanVar <- w %>% group_by(PC, term) %>% summarise_at(vars('breed', 'without breed'), mean)
head(meanVar)
meanVar <- pivot_longer(meanVar, cols=c(breed, `without breed`), names_to='model', values_to='meanVar')
head(meanVar)

ggplot(meanVar, aes(y=meanVar, x=model, color=model, group=term))+
  geom_line(col='grey')+
  geom_point()+
  theme_classic(base_size = 14)+
...
meanVar <- w %>% group_by(term) %>% summarise_at(vars('breed', 'without breed'), mean)
head(meanVar)
meanVar <- pivot_longer(meanVar, cols=c(breed, `without breed`), names_to='model', values_to='meanVar')
...
differenceFactetPlot <- ggplot(w, aes(y=difference, x=PC, fill=term))+
  ...
  theme(axis.text.y=element_text(size=7))+
  theme(legend.position = 'none')+
  ylab('change in variance explained (V with breed - V without breed)')+
  xlab('')+
  coord_flip()+
  facet_wrap(~term)+
  scale_x_discrete(limits=rev)
...
meanDiff <- aggregate(difference ~ term, w, mean)


diffBoxPlot <- ggplot(w, aes(y=difference, x=term, fill=term, color=term))+
  geom_abline(intercept = 0, slope=0, color='grey20', linetype='dashed')+
  geom_boxplot(alpha=0.5)+
  geom_point()+
  theme_classic(base_size = 20)+
  theme(legend.position = 'none')+
  ylab('change in variation exp')+
  xlab('')+
  coord_flip()+
  scale_x_discrete(limits=rev)


diffBarPlot <- ggplot(w, aes(y=difference, x=term, fill=term))+
  geom_abline(intercept = 0, slope=0, color='grey20', linetype='dashed')+
  geom_bar(stat = "summary", alpha=0.9)+
  theme_classic(base_size = 20)+
  theme(legend.position = 'none')+
  ylab('change in variation exp (mean of 18 PCs)')+
  xlab('')+
  coord_flip()+
  scale_x_discrete(limits=rev)

differenceFactetPlot
ggarrange(diffBarPlot, diffBoxPlot, ncol=1)

ggarrange(differenceFactetPlot, ggarrange(diffBarPlot, diffBoxPlot, ncol=1))
...
removeBreedPlot <- ggplot(subset(w, PC %in% breedPCs), aes(y=difference, x=term, fill=term, color=term))+
  geom_abline(intercept = 0, slope=0, color='grey20', linetype='dashed')+
  geom_boxplot(alpha=0.5)+
  geom_point()+
  theme_classic(base_size = 16)+
...
removeGRMplot <- removeGRMplot + xlab('') + ylab('mean variance (5 breed PCs)')
removeBreedPlot <- removeBreedPlot + ylab('change in PC variance explained')

ggarrange(removeGRMplot, removeBreedPlot, ncol=1, heights = c(1, 0.5))
