...
u <- read.csv('/Users/ben/Library/CloudStorage/GoogleDrive-brharrison2000@gmail.com/My Drive/Documents/DogAgingProject/data/2023_release/DAP_2023_SamplesResults_Urine_v1.0.csv')
head(u)
str(u)
...
betas['krt_urine_bilirubin', ] # bilirubine is negatively assoc. with age
...
ggplot(u, aes(y=Ornithine, x=krt_urine_protien))+
  geom_point( )+
  geom_smooth()+
...
load("data/metabolome/ProcessedData/P1.technicalCovarsRemovedData") # get the unscaled age
rownames(dat) <- dat$dog_id
dat <- dat[u$dog_id, ]

u$unscaledsqrtAge <- sqrt(dat$age)

p1 <- ggplot(u, aes(x=unscaledsqrtAge, y=krt_urine_sg))+
...
l <- scu %>% pivot_longer(cols=all_of(uSGmzs), names_to = 'uSGmz')
l$uSGmz <- factor(l$uSGmz, levels=uSGmzs)

ggplot(l, aes(y=value, x=krt_urine_sg))+
  geom_smooth(method='lm')+
  geom_point(size=0.5, alpha=0.5)+
  theme_classic(base_size = 12)+
  facet_wrap(~uSGmz, scales='free', ncol=5)

ggplot(scu, aes(x=sqrtAge, y=krt_urine_sg))+
  geom_smooth(method='lm')+
  geom_point(size=1, alpha=0.5)+
  theme_classic(base_size = 12)


ggplot(scu, aes(x=sqrtAge, y=`Glycerol-3-P`))+
  geom_smooth(method='lm')+
  geom_point(size=1, alpha=0.5)+
  theme_classic(base_size = 12)

ggplot(scu, aes(x=sqrtAge, y=Hydroxyproline))+
  geom_smooth(method='lm')+
  geom_point(size=1, alpha=0.5)+
  theme_classic(base_size = 12)
...
table(out$sensitivity) # all causal mediation models fail the sensitivity test

out[out$ACME_FDR<=0.05, ] # after scaling uSG, it no longer medaites the effect of age on any MZs (before scaling it mediated age on mz for almost all of 3 mzs - so, that was an artifact)



rm(list=ls())
##################################################################################
#### mediation analysis: reverse path
# test reverse mediation path: Is uSG ~ age driven by any mzs?
##################################################################################
...
clusterExport(clus, varlist=c('scu', 'dogmzs'), envir = environment()) 

for(i in 1:length(dogmzs)) {
u$MZ <- scu[ ,dogmzs[i]]
clusterExport(clus, varlist=c('testUvars', 'u', 'dogmzs'), envir = environment()) 
med.fit <- lm(krt_urine_sg ~ sqrtAge + sqrtWT + hours_fasting + sex * sterilization_status + method + volume, data = scu) 
out.fit <- lm(krt_urine_sg ~ MZ + sqrtAge + sqrtWT + hours_fasting + sex * sterilization_status + method + volume, data = scu)
revModList[[i]] <- mediate(med.fit, out.fit, treat = "sqrtAge", mediator = "MZ",  sims=1000, boot=T) }

names(revModList) <- dogmzs
sensList <- lapply(revModList, function(x) medsens(x))
sumList <- lapply(revModList, function(x) summary(x))
out <- lapply(sumList, medOut)
out <- do.call(rbind, out)
out$mz <- dogmzs
out$ADE_FDR <- p.adjust(out$ADE_P, 'fdr')
out$ACME_FDR <- p.adjust(out$ACME_P, 'fdr')
out$sensitivity <- sapply(sensList, sensitiveTest)
out$mediator <- 'metabolites'

save(out, revModList, sensList, file='mediationOfUrineSG_byMetabolites')


load('mediationOfUrineSG_byMetabolites')
